...
# Pegando ROI (Região de interesse)
# x, y - width, height [Retângulo] 
vaga1 = [1, 89, 108, 213]
...
video = cv2.VideoCapture('arquivo/video.mp4')

while video.isOpened():
  check, img = video.read()
  
  #Check retorna um booleano - Se video.read() retorna ou não uma img 
  if not check:
    break

  imgCinza = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) 
  #Limite ignorando sobrandos e ruidos - img, valorMaximo = 255 (Padrão), método, tipo de threshold, blocksize, C (São variantes|Não sei sobre esse últimos)
  ...
  for x, y, w, h in vagas:
    ...
    if qtPxBranco > 3000:
      #Desenhando retângulo - img, posição, cor, espessura
  ...
  #Exibir quantia de vagas // -1 no final é para pintar ele todo
